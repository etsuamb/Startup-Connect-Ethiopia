...
function normalizeConversation(row) {
	...
	const founder = [
		row.startup_contact_first_name,
		row.startup_contact_last_name,
	]
		.filter(Boolean)
		.join(" ");
	return {
...
export default function MentorMessagesPage() {
		...
		async (preferredStartupId = null, preferredConversationId = null) => {
			...
			setSelected((current) => {
				if (preferredConversationId) {
					return (
						list.find(
							(item) => String(item.id) === String(preferredConversationId),
						) ||
						current ||
						list[0] ||
						null
				...
				if (preferredStartupId) {
					return (
						list.find(
							(item) => String(item.startupId) === String(preferredStartupId),
						) ||
						current ||
						list[0] ||
						null
		...
		setMessages(
			decrypted.map((message) => ({
				...
				display_body:
					...
					(isEncryptedChatPayload(message.text_body)
						? "[Encrypted message]"
						: message.text_body || ""),
			})),
		);
			...
			try {
				...
				if (startupIdFromUrl) {
					try {
						...
						setError(
							isChatAccessError(ex)
								? chatAccessMessage(ex)
								: ex.message || "Failed to open conversation.",
						);
					}
				}
				...
				setError(
					isChatAccessError(ex)
						? chatAccessMessage(ex)
						: ex.message || "Failed to load conversations.",
				);
			} finally {
				setLoading(false);
			}
	...
	async function openFile(message) {
		try {
			const data = await downloadMentorChatFile(
				selected.id,
	...
	async function send(event) {
		...
		try {
			...
			setError(
				isChatAccessError(ex)
					? chatAccessMessage(ex)
					: ex.message || "Message could not be sent.",
			);
		} finally {
			setSending(false);
		}
					...
					<aside className="flex w-[380px] shrink-0 flex-col border-r border-gray-100 bg-white">
						...
						<div className="min-h-0 flex-1 overflow-y-auto">
							{loading ? (
										...
										>
											...
											<div className="min-w-0 flex-1">
												...
												<div className="mt-2 flex items-center gap-2">
													<p
														className={`truncate text-sm ${conversation.unread ? "font-black text-gray-950" : "text-gray-500"}`}
													>
								...
								<p className="p-5 text-sm text-gray-500">
									No real mentor-startup conversations yet. Accepted mentorships
									will appear here.
								</p>
							)}
						</div>
							...
							<>
								<div className="flex h-[86px] shrink-0 items-center justify-between border-b border-gray-100 bg-white px-8">
									...
									<div className="flex shrink-0 items-center gap-3">
										...
										<Link
											href={`/mentor/sessions?startupId=${selected.startupId}`}
											className="rounded-lg bg-[#073f32] px-4 py-2 text-xs font-black text-white hover:bg-[#052f26]"
										>
											Schedule Session
										</Link>
										<button
											...
											className="rounded-lg border border-gray-200 bg-white p-2 text-gray-500 hover:text-[#073f32]"
											title="Audio call"
										>
											<svg
												className="h-5 w-5"
											...
											>
												<path
													...
													strokeLinejoin="round"
													strokeWidth="2"
													d="M3 5a2 2 0 012-2h3.3a1 1 0 011 .7l1.4 4.5a1 1 0 01-.5 1.2L6.7 10.5a11 11 0 005.8 5.8l1.1-2.5a1 1 0 011.2-.5l4.5 1.4a1 1 0 01.7 1V19a2 2 0 01-2 2h-1C9.3 21 3 14.7 3 7V5z"
												/>
											</svg>
										</button>
										<button
										...
										>
											...
											>
												<path
													...
													strokeLinejoin="round"
													strokeWidth="2"
													d="M15 10l4.6-2.3A1 1 0 0121 8.6v6.8a1 1 0 01-1.4.9L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"
												/>
											</svg>
										</button>
									</div>
								...
								{error ? (
									<div
										className={`mx-8 mt-4 rounded-xl border px-4 py-3 text-sm font-semibold ${
								...
								<div className="min-h-0 flex-1 overflow-y-auto px-10 py-8">
									{messagesLoading ? (
											...
											{groupedMessages().map((item, index) => {
												if (item.type === "day") {
													return (
														...
														>
															<span className="rounded-full border border-gray-200 bg-white px-5 py-1.5 text-[10px] font-black uppercase tracking-widest text-gray-400">
																{item.label}
															</span>
														</div>
													);
												...
												return (
													<div
														key={message.mentor_chat_message_id || index}
														className={`flex ${mine ? "justify-end" : "justify-start"}`}
													>
														<div
															className={`max-w-[68%] ${mine ? "text-right" : "text-left"}`}
														>
															<div
																className={`rounded-2xl px-5 py-4 shadow-sm ${mine ? "rounded-tr-sm bg-[#073f32] text-white" : "rounded-tl-sm border border-gray-100 bg-white text-gray-950"}`}
															>
																{isFile ? (
																	<button
																		type="button"
																	...
																	>
																		...
																		<span className="min-w-0 flex-1">
																			...
																			<span
																				className={`mt-1 block text-xs ${mine ? "text-white/65" : "text-gray-500"}`}
																			>
																				{bytes(message.file_size_bytes)} - Press
																				to view
																			</span>
																		</span>
																	</button>
																	...
																	<p className="whitespace-pre-wrap text-sm leading-7">
																		{message.display_body || message.text_body}
																	</p>
																)}
																{isFile && message.display_body ? (
										...
										<div className="flex h-full items-center justify-center text-sm text-gray-500">
											No messages yet. Start the conversation with this startup.
										</div>
									)}
								</div>
										...
										>
											...
											>
												<path
													...
													strokeLinejoin="round"
													strokeWidth="2"
													d="M15.2 7l-6.6 6.6a2 2 0 102.8 2.8l6.4-6.6a4 4 0 00-5.6-5.6l-6.4 6.6a6 6 0 108.5 8.5l6.2-6.2"
												/>
											</svg>
										</button>
										<input
