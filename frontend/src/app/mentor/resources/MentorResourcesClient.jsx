...
import { fetchMentorResources, fetchMyStartups, shareResource } from "@/lib/mentorApi";
...
export default function MentorResourcesPage() {
	...
	const load = useCallback(async () => {
		...
		try {
			...
			const loadedResources = Array.isArray(resourceData) ? resourceData : resourceData.resources || [];
			...
			setForm((current) => ({
				...current,
				mentorship_request_id:
					current.mentorship_request_id || loadedStartups[0]?.mentorship_request_id || "",
			}));
		} catch (ex) {
	...
	const selectedStartup = useMemo(
		() => startups.find((startup) => String(startup.mentorship_request_id) === String(form.mentorship_request_id)) || null,
	...
	const previewResource = useMemo(() => {
		...
		return {
			title: form.resource_title || latest?.resource_title || "Resource preview",
			description: form.resource_description || latest?.resource_description || "A short description will appear here.",
			type: form.resource_type,
			startup: selectedStartup?.startup_name || latest?.startup_name || "Selected startup",
		};
	}, [form.resource_description, form.resource_title, form.resource_type, resources, selectedStartup]);
	...
	async function submit(event) {
		...
		try {
			...
			fd.append(
				"resource_description",
				[form.resource_description, form.note ? `Mentor note: ${form.note}` : "", form.category ? `Category: ${form.category}` : ""]
					.filter(Boolean)
					.join("\n\n"),
			);
		...
		<div className="min-h-full bg-[#f6f7f8]">
			<header className="flex h-[62px] items-center justify-between border-b border-gray-100 bg-white px-8">
				...
				<div className="flex items-center gap-4 text-gray-400">
					...
					<span className="text-xs font-black text-gray-900">StartupConnect Ethiopia</span>
				</div>
			</header>

			<main className="mx-auto w-full max-w-[1040px] px-6 py-8">
				<div className="mb-5">
					<h1 className="text-2xl font-black text-gray-950">Provide Learning Resources</h1>
					<p className="mt-1 text-xs text-gray-500">
						Share guides, templates, links, and documents to help startups improve their business.
					</p>
				</div>
				...
				{success ? <div className="mb-4 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-700">{success}</div> : null}
				...
				<div className="grid grid-cols-1 gap-5 lg:grid-cols-[1fr_300px]">
					<div className="space-y-5">
						<section className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
							...
							<div className="grid gap-4 md:grid-cols-[1fr_280px]">
								<select
									...
									onChange={(event) => update("mentorship_request_id", event.target.value)}
								...
								>
									...
									{startups.map((startup) => (
										<option key={startup.mentorship_request_id} value={startup.mentorship_request_id}>
								...
								<div className="rounded-lg bg-gray-50 p-3 text-xs">
									...
									<div className="flex items-center justify-between gap-3">
										<p className="font-black text-gray-950">{selectedStartup?.industry || "Not specified"}</p>
										<p className="font-black text-[#073f32]">{selectedStartup?.business_stage || "Active"}</p>
									</div>
									<p className="mt-2 font-semibold text-gray-600">{selectedStartup?.subject || selectedStartup?.startup_tagline || "Accepted mentorship"}</p>
								</div>
							</div>
						</section>
						...
						<form onSubmit={submit} className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
							...
							<div className="space-y-4">
								<div>
									<label className="mb-1.5 block text-[11px] font-bold text-gray-600">Resource Title</label>
									<input
										value={form.resource_title}
										onChange={(event) => update("resource_title", event.target.value)}
								...
								<div className="grid gap-4 md:grid-cols-2">
									<div>
										<label className="mb-1.5 block text-[11px] font-bold text-gray-600">Resource Type</label>
										<select
									...
									<div>
										<label className="mb-1.5 block text-[11px] font-bold text-gray-600">Category</label>
										<select
											value={form.category}
								...
								<div>
									<label className="mb-1.5 block text-[11px] font-bold text-gray-600">Resource Link (Optional)</label>
									<input
										type="url"
								...
								<div>
									<label className="mb-1.5 block text-[11px] font-bold text-gray-600">Upload File</label>
									<button
										type="button"
									...
									>
										<svg className="mb-2 h-7 w-7 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
										...
										<span>{file ? file.name : "Drag and drop files here, or browse"}</span>
										<span className="mt-1 text-[10px] text-gray-400">PDF, DOCX, PPTX, PNG, JPG (Max 25MB)</span>
									</button>
									<input ref={fileRef} type="file" className="hidden" onChange={(event) => setFile(event.target.files?.[0] || null)} />
								</div>

								<div>
									<label className="mb-1.5 block text-[11px] font-bold text-gray-600">Short Description</label>
									<textarea
										value={form.resource_description}
										onChange={(event) => update("resource_description", event.target.value)}
								...
								<div>
									<label className="mb-1.5 block text-[11px] font-bold text-gray-600">Notes for Startup</label>
									<textarea
										value={form.note}
							...
							<div className="mt-6 flex items-center justify-end gap-3 border-t border-gray-100 pt-5">
								<button type="button" onClick={resetForm} className="px-4 py-2 text-xs font-bold text-gray-500">Cancel</button>
								<button type="button" className="rounded-lg border border-gray-300 bg-white px-4 py-2 text-xs font-bold text-gray-700">Save as Draft</button>
								<button disabled={saving || startups.length === 0} className="rounded-lg bg-[#073f32] px-5 py-2 text-xs font-black text-white disabled:opacity-50">
						...
						<section className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
							<div className="mb-4 flex items-center justify-between">
								<h2 className="text-sm font-black">Previously Shared Resources</h2>
								<span className="text-xs font-black text-[#073f32]">View All</span>
							</div>
							<div className="overflow-hidden">
								<table className="w-full text-left text-xs">
									...
									<tbody className="divide-y divide-gray-100">
										{loading ? (
											<tr><td colSpan="5" className="py-6 text-gray-500">Loading resources...</td></tr>
										) : resources.length ? resources.slice(0, 5).map((resource) => {
											const status = resourceStatus(resource);
											return (
												<tr key={resource.resource_id}>
													<td className="py-3">
														<p className="font-black text-gray-900">{resource.resource_title}</p>
														<p className="text-[10px] text-gray-400">{resource.file_name || resource.external_url || "Shared note"}</p>
													</td>
													<td className="py-3 font-semibold text-gray-600">{resource.startup_name || "Startup"}</td>
													<td className="py-3">
														<span className="rounded bg-orange-50 px-2 py-1 text-[10px] font-black uppercase text-orange-700">{shortType(resource.resource_type)}</span>
													</td>
													<td className="py-3 text-gray-500">{formatDate(resource.created_at)}</td>
													<td className="py-3 text-right">
														<span className={`rounded px-2 py-1 text-[10px] font-black ${statusClass(status)}`}>{status}</span>
													</td>
												</tr>
											);
										}) : (
											<tr><td colSpan="5" className="py-6 text-gray-500">No resources shared yet.</td></tr>
										)}
									</tbody>
								</table>
							</div>
					...
					<aside className="space-y-5">
						<section className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
							...
							<div className="mt-3">
								<span className="rounded bg-emerald-100 px-2 py-1 text-[9px] font-black uppercase text-[#073f32]">{shortType(previewResource.type)}</span>
								<p className="mt-3 text-sm font-black text-gray-950">{previewResource.title}</p>
								<p className="mt-1 line-clamp-3 text-xs leading-5 text-gray-500">{previewResource.description}</p>
								<div className="mt-4 flex items-center justify-between border-t border-gray-100 pt-3 text-[10px] text-gray-400">
									...
									<span>{file ? bytes(file.size) : form.external_url ? "External link" : "Ready"}</span>
								</div>
							</div>
						</section>
						...
						<section className="rounded-xl bg-[#073f32] p-5 text-white shadow-sm">
							...
							<p className="mt-1 text-xs text-white/70">Recently shared resources from your backend records.</p>
							<div className="mt-4 space-y-2">
								{quickResources.length ? quickResources.map((resource) => (
									<div key={resource.resource_id} className="flex items-center justify-between rounded-lg bg-white/10 px-3 py-2 text-xs font-black">
										<span className="truncate">{resource.resource_title}</span>
										<span className="ml-3 text-white/50">+</span>
									</div>
								)) : (
									<p className="rounded-lg bg-white/10 px-3 py-3 text-xs text-white/70">Shared resources will appear here.</p>
								)}
							</div>
						</section>
