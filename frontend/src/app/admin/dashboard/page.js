...
const STAT_CARDS = [
	{ labelKey: "dashboard.totalUsers", valueKey: "total_users", accent: "border-l-[#006054]" },
	{ labelKey: "dashboard.verifiedStartups", valueKey: "total_startups", accent: "border-l-[#0f766e]" },
	{ labelKey: "dashboard.investors", valueKey: "total_investors", accent: "border-l-[#134e4a]" },
	{ labelKey: "dashboard.mentors", valueKey: "total_mentors", accent: "border-l-[#115e59]" },
];

const METRIC_CARDS = [
	{ labelKey: "dashboard.verifiedUsers", valueKey: "total_verified_users", accent: "border-l-[#047857]" },
	{
	...
	{ labelKey: "dashboard.mentorshipPayments", valueKey: "total_mentorship_transactions", accent: "border-l-[#0d9488]" },
	{ labelKey: "dashboard.investmentPayments", valueKey: "total_investment_transactions", accent: "border-l-[#14b8a6]" },
];
		...
		<div className="max-w-7xl mx-auto pb-12">
			...
			<section className="mb-8 rounded-[32px] bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white shadow-[0_20px_60px_rgba(15,23,42,0.2)] overflow-hidden">
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-6 p-8 md:p-10">
					<div>
						<p className="text-sm uppercase tracking-[0.28em] text-slate-300 mb-4">{t("dashboard.adminOverview")}</p>
					...
					<div className="flex flex-col justify-between gap-4 rounded-[28px] bg-slate-950/80 border border-slate-700 p-6">
						<div>
							<p className="text-xs uppercase tracking-[0.28em] text-slate-400">{t("dashboard.currentStatus")}</p>
							<p className="mt-2 text-lg font-semibold text-white">{dbOk ? t("dashboard.healthy") : t("dashboard.attentionNeeded")}</p>
						</div>
						<span className={`inline-flex items-center rounded-full px-3 py-1 text-sm font-semibold ${dbOk ? "bg-emerald-500/15 text-emerald-200" : "bg-amber-500/15 text-amber-200"}`}>
							{dbOk ? t("dashboard.databaseConnected") : t("dashboard.databaseUnavailable")}
						</span>
						<div className="rounded-3xl bg-slate-900/90 p-4">
							<p className="text-xs uppercase tracking-[0.28em] text-slate-500">{t("dashboard.lastUpdated")}</p>
			...
			<div className="mb-6">
				...
				{analyticsTab === "investments" && fundingStats ? (
					<div className="bg-white rounded-2xl border p-4 text-sm text-slate-600">
						<p>{t("dashboard.totalRequests")}: {fundingStats.total_requests ?? fundingStats.total_funding_requests ?? "—"}</p>
						<p>{t("dashboard.approved")}: {fundingStats.approved ?? "—"} · {t("dashboard.pending")}: {fundingStats.pending ?? "—"}</p>
					</div>
				) : null}
				{analyticsTab === "engagement" && engagement ? (
					<div className="bg-white rounded-2xl border p-4 text-sm">
						<p>{t("dashboard.activeUsersRecent")}: {engagement.active_users ?? engagement.total_active ?? "—"}</p>
					</div>
				) : null}
				{analyticsTab === "users" && stats ? (
					<div className="bg-white rounded-2xl border p-4 text-sm grid sm:grid-cols-2 gap-2">
						<p>{t("dashboard.totalUsers")}: {stats.total_users}</p>
						<p>{t("dashboard.verified")}: {stats.total_verified_users}</p>
						<p>{t("dashboard.pendingApproval")}: {stats.pending_users}</p>
						<p>{t("dashboard.active")}: {stats.active_users}</p>
					</div>
				) : null}
			</div>
			...
			<div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4 mb-8">
				{STAT_CARDS.map((card) => (
					...
					>
						<p className="text-xs uppercase tracking-[0.2em] text-slate-500 font-semibold">{t(card.labelKey)}</p>
			...
			<div className="grid grid-cols-1 gap-8 xl:grid-cols-[1.45fr_0.95fr] mb-8">
				<div className="rounded-[32px] bg-white border border-slate-200 shadow-sm p-8">
					<div className="flex items-center justify-between gap-4 mb-8">
						<div>
							<p className="text-xs uppercase tracking-[0.28em] text-slate-400">{t("dashboard.platformPulse")}</p>
							<h2 className="mt-3 text-2xl font-semibold text-slate-900">{t("dashboard.approvalOverview")}</h2>
						</div>
						<span className="inline-flex items-center rounded-full bg-emerald-500/10 px-3 py-1 text-sm font-semibold text-emerald-700">{t("dashboard.operational")}</span>
					</div>

					<div className="grid grid-cols-1 gap-5 md:grid-cols-2">
						<div className="rounded-3xl bg-slate-50 p-6 border border-slate-100">
							<p className="text-sm font-semibold text-slate-900">{t("dashboard.pendingVerifications")}</p>
							<p className="mt-4 text-4xl font-bold text-slate-900">{loading ? "—" : pendingCount}</p>
							<p className="mt-2 text-sm text-slate-500">{t("dashboard.pendingVerificationsHelp")}</p>
						</div>
						<div className="rounded-3xl bg-slate-50 p-6 border border-slate-100">
							<p className="text-sm font-semibold text-slate-900">{t("dashboard.activeUserVolume")}</p>
							<p className="mt-4 text-4xl font-bold text-slate-900">{loading ? "—" : s.active_users ?? 0}</p>
							<p className="mt-2 text-sm text-slate-500">{t("dashboard.activeUserVolumeHelp")}</p>
						</div>
						<div className="rounded-3xl bg-slate-50 p-6 border border-slate-100">
							<p className="text-sm font-semibold text-slate-900">{t("dashboard.projectsListed")}</p>
							<p className="mt-4 text-4xl font-bold text-slate-900">{loading ? "—" : s.total_projects ?? 0}</p>
							<p className="mt-2 text-sm text-slate-500">{t("dashboard.projectsListedHelp")}</p>
						</div>
						<div className="rounded-3xl bg-slate-50 p-6 border border-slate-100">
							<p className="text-sm font-semibold text-slate-900">{t("dashboard.fundingRequests")}</p>
							<p className="mt-4 text-4xl font-bold text-slate-900">{loading ? "—" : s.total_funding_requests ?? 0}</p>
							<p className="mt-2 text-sm text-slate-500">{t("dashboard.fundingRequestsHelp")}</p>
						</div>
					</div>
				</div>
				...
				<div className="rounded-[32px] bg-white border border-slate-200 shadow-sm p-8">
					<div className="flex items-center justify-between gap-4 mb-6">
						<div>
							<p className="text-xs uppercase tracking-[0.28em] text-slate-400">{t("dashboard.actionCenter")}</p>
							<h2 className="mt-3 text-2xl font-semibold text-slate-900">{t("dashboard.adminControls")}</h2>
						</div>
						<span className="rounded-full bg-slate-100 px-3 py-1 text-sm text-slate-600">{t("dashboard.priority")}</span>
					</div>
					<div className="space-y-4">
						{visibleActionLinks.map((item) => (
							...
							>
								...
								<p className="text-sm text-slate-500 mt-1">{t(item.descriptionKey)}</p>
							</Link>
						))}
					</div>
					<div className="mt-6 flex items-center justify-between border-t border-slate-100 pt-4">
						<p className="text-xs text-slate-500">
							{t("dashboard.page", { current: actionPage + 1, total: actionTotalPages })}
						</p>
