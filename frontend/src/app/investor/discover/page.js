		...
		<div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm hover:shadow-md transition flex flex-col lg:flex-row gap-5 lg:items-center">
			<div className="w-full lg:w-20 h-20 bg-[#0d3328] rounded-xl flex items-center justify-center shrink-0">
				<ActorAvatar
					...
					initials={initials(startup.startup_name)}
					className="h-12 w-12 rounded-xl text-sm font-black"
			...
			<div className="lg:text-right shrink-0">
				...
				<p className="text-2xl font-bold text-[#0a4d3c] mb-4">
					{formatCurrency(startup.funding_needed)}
				</p>
				<Link
					href={profileHref}
					className="inline-flex px-5 py-2.5 bg-[#0a4d3c] text-white text-sm font-bold rounded-lg hover:bg-[#07382b] transition"
				>
					View Profile
				</Link>
				<Link
					href={`/investor/offers/new?startupId=${startup.startup_id}`}
					className="inline-flex px-5 py-2.5 mt-2 lg:ml-2 bg-white border border-gray-200 text-gray-700 text-sm font-bold rounded-lg hover:bg-gray-50 transition"
				>
					Send Offer
				</Link>
			</div>
...
export default function InvestorDiscover() {
	...
	const stageOptions = useMemo(() => {
		const fromResults = startups
			.map((startup) => startup.business_stage)
			.filter(Boolean);
					...
					<div className="max-w-[1200px] mx-auto flex flex-col min-h-full">
						<div className="mb-8">
							...
							<p className="text-gray-500 text-[15px]">
								Discover high-potential investment opportunities in Ethiopia.
							</p>
						</div>
						...
						<div className="bg-white border border-gray-200 rounded-xl p-6 mb-8 flex flex-wrap lg:flex-nowrap gap-6 items-end shadow-sm">
							<div className="flex-1 min-w-[200px]">
								...
								<select
									...
									onChange={(event) =>
										updateFilter("industry", event.target.value)
									}
							...
							<button
								...
								className="w-full lg:w-auto px-8 py-3 bg-[#0a4d3c] text-white text-sm font-bold rounded-lg hover:bg-[#07382b] transition shadow-sm"
							>
								Apply Filters
							</button>
						</div>
