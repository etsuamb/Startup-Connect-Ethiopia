...
function normalizeMentorConversation(row) {
  const name = `${row.mentor_first_name || ""} ${row.mentor_last_name || ""}`.trim() || "Mentor";
  return {
    id: row.mentor_conversation_id,
    name,
    subtitle: row.mentor_headline || row.mentor_email || "Accepted mentorship partner",
...
export default function StartupMentorChatPage() {
  ...
  async function loadConversations() {
    try {
      setLoading(true);
      setError(null);
      const data = await getMentorChatConversations();
      const list = (data.conversations || []).map(normalizeMentorConversation);
      ...
      setError(err.message || "Unable to load accepted mentor conversations.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function fetchMessages(conversationId) {
    try {
      ...
      setError(null);
      const data = await getMentorChatMessages(conversationId);
  ...
  async function handleSendWithFile(event) {
    ...
    if (selectedFile) {
      ...
      try {
        const data = await sendMentorChatFile(selected.id, formData);
      ...
      setMessageText("");
      try {
        const data = await sendMentorChatMessage(selected.id, text);
        setMessages((current) => [...current, data.message]);
        await loadConversations();
      } catch (err) {
  ...
  function messageKey(message, index) {
    return message.mentor_chat_message_id || `${message.created_at}-${index}`;
  }
      ...
      <main className="flex-grow flex flex-col overflow-y-auto">
        <header className="px-8 py-6 bg-white border-b border-gray-100 sticky top-0 z-10">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Message your mentors and run voice or video sessions after mentorship is accepted.
          </p>
        </header>
          ...
          <div className="grid gap-6 lg:grid-cols-[340px_1fr]">
            <aside className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
              <div className="border-b border-gray-100 p-5">
                ...
                <p className="mt-1 text-sm text-gray-500">Accepted mentors only</p>
              </div>
              {loading ? (
                <p className="p-5 text-sm text-gray-500">Loading conversations...</p>
              ) : conversations.length === 0 ? (
                <p className="p-5 text-sm text-gray-500">
                  No accepted mentor chats yet. Conversations appear after a mentorship offer or request is accepted.
                </p>
              ) : (
            ...
            <section className="rounded-xl border border-gray-200 bg-white shadow-sm flex min-h-[640px] flex-col overflow-hidden">
              <div className="border-b border-gray-100 p-5 space-y-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500">{selected?.subtitle || "Choose an accepted mentor."}</p>
                </div>
                <ChatCallPanel
                ...
                <div className="flex gap-3">
                  <div className="relative flex-1">
                    ...
                    <div className="absolute right-2 top-1/2 -translate-y-1/2 flex gap-1">
                      <input type="file" id="mentor-file-upload" className="hidden" onChange={handleFileSelect} disabled={!selected} />
                      <label
