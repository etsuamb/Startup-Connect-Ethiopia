...
function pagePlaceholder(pathname) {
	...
	if (pathname?.includes("/startups")) return "Search startups, founders, industries...";
...
function MentorTopbar() {
	...
	const name = `${profile?.first_name || ""} ${profile?.last_name || ""}`.trim() || profile?.full_name || "Mentor";
	const title = profile?.headline || profile?.professional_title || "Lead Mentor";
