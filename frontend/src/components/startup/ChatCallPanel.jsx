  ...
  return (
    <div className="rounded-xl border border-[#0f3d32]/20 bg-gradient-to-br from-[#f0faf5] to-white overflow-hidden">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#0f3d32]/10 px-4 py-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-widest text-[#0f3d32]">Video & voice session</p>
          <p className="text-sm text-gray-600 mt-0.5">With {partnerName || "partner"}</p>
        </div>
        <span
      ...
      {incomingCall && (
        <div className="mx-4 mt-4 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3">
          <p className="text-sm font-semibold text-blue-900">Incoming call from {partnerName}</p>
        ...
        <div className="grid gap-3 px-4 pb-4 sm:grid-cols-2">
          <div className="overflow-hidden rounded-xl border border-gray-200 bg-black min-h-[180px]">
            {mediaMode === "video" && localStream ? (
              <video ref={localVideoRef} autoPlay muted playsInline className="h-full w-full min-h-[180px] object-cover" />
            ) : (
              <div className="flex h-full min-h-[180px] flex-col items-center justify-center gap-2 text-white">
                ...
                <p className="text-sm font-semibold">{isInCall ? "Voice session active" : "Waiting to connect media…"}</p>
              </div>
            )}
            <p className="bg-black/70 px-2 py-1 text-center text-[10px] font-bold uppercase tracking-wider text-white/80">You</p>
          </div>

          <div className="overflow-hidden rounded-xl border border-gray-200 bg-[#1a1a1a] min-h-[180px]">
            {screenStream ? (
              <video ref={screenVideoRef} autoPlay muted playsInline className="h-full w-full min-h-[180px] object-contain" />
            ) : screenShareUserId && Number(screenShareUserId) !== Number(currentUserId) ? (
              <div className="flex h-full min-h-[180px] flex-col items-center justify-center gap-2 px-4 text-center text-white">
                <p className="text-sm font-semibold">{partnerName} is sharing their screen</p>
                <p className="text-xs text-white/60">Room: {callState.video_call?.room_id?.slice(0, 8)}…</p>
              </div>
            ) : (
