...
exports.createOrGetConversation = async (req, res) => {
	try {
		...
		if (role === "Startup") {
			...
			if (!(await requireAcceptedInvestment(s.startup_id, invId, res))) return;
		...
		if (role === "Investor") {
			...
			if (!(await requireAcceptedInvestment(sid, inv.investor_id, res))) return;
			const ex = await pool.query(
				"SELECT * FROM chat_conversations WHERE startup_id = $1 AND investor_id = $2",
				[sid, inv.investor_id],
			);
...
exports.videoStart = async (req, res) => {
	...
	try {
       ...
       RETURNING *`,
		...
		await pool.query(
			`INSERT INTO notifications (user_id, notification_type, title, message, reference_type, reference_id)
       VALUES ($1, 'video', 'Incoming video call', $2, 'chat_video_calls', $3)`,
