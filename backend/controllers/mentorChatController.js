...
async function getMentorByUserId(userId) {
	const r = await pool.query("SELECT mentor_id, user_id FROM mentors WHERE user_id = $1", [userId]);
	return r.rows[0] || null;
}
...
function isParticipant(conv, userId) {
	...
	return Number(conv.startup_user_id) === uid || Number(conv.mentor_user_id) === uid;
}
	...
	try {
		...
		if (role === "Startup") {
			...
			if (!Number.isInteger(mid) || mid <= 0) {
				return res.status(400).json({ error: "mentor_id is required for Startup" });
			}
		...
		if (role === "Mentor") {
			...
			if (!ment) return res.status(403).json({ error: "Mentor profile required" });
			const sid = Number(startup_id);
			if (!Number.isInteger(sid) || sid <= 0) {
				return res.status(400).json({ error: "startup_id is required for Mentor" });
			}
		...
		return res.status(403).json({ error: "Only Startup or Mentor may use mentor chat" });
	} catch (err) {
...
exports.listConversations = async (req, res) => {
	try {
       ...
       ORDER BY c.last_message_at DESC NULLS LAST, c.created_at DESC`,
			...
			return res.status(403).json({ error: "Only Startup or Mentor may list mentor conversations" });
		}
...
exports.sendTextMessage = async (req, res) => {
	try {
		...
		const body = req.body || {};
		const text =
			typeof body.body === "string" ? body.body : typeof body.text === "string" ? body.text : "";
		if (!text.trim()) {
       ...
       VALUES ($1, 'mentor_chat', 'New mentor chat message', $2, 'mentor_chat_conversations', $3)`,
			[peerUserId, text.trim().slice(0, 200) || "You have a new message", convId],
		);
...
async function ensureMentorVideoParticipantJoined(client, mentorVideoCallId, userId) {
	await client.query(
...
async function closeMentorParticipantSessionsForCall(client, mentorVideoCallId) {
	await client.query(
		`UPDATE mentor_chat_video_session_participants SET left_at = CURRENT_TIMESTAMP
...
exports.videoJoin = async (req, res) => {
	try {
       ...
       ORDER BY created_at DESC LIMIT 1`,
		...
		if (!r.rowCount) {
			return res.status(404).json({ error: "No active or ringing call for this conversation" });
		}
